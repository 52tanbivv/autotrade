...
public class WxPayHelper {
	...
	public String GetCftPackage() throws SDKRuntimeException {
		...
		// String encodeString = "";
		// try {
		// encodeString = new String(unSignParaString.getBytes("utf-8"), "gbk");
		// } catch (UnsupportedEncodingException e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
		// }
		// System.out.println("GetCftPackage="
		// + MD5SignUtil.Sign(encodeString, PartnerKey));
	...
	public String GetBizSign(HashMap<String, String> bizObj)
		...
		for (int i = 0; i < infoIds.size(); i++) {
			...
			if (item.getKey() != "") {
				...
				// if ("RetErrMsg".equals(item.getKey())) {
				// try {
				// if (item.getValue().equals("已付费")) {
				// val = "haspay";
				// } else {
				// val = new String(item.getValue().getBytes(),
				// "utf-8");
				// }
				//
				// } catch (UnsupportedEncodingException e) {
	 ...
	 */
	public HashMap<String, String> CreateBizPackage()
			throws SDKRuntimeException {
		HashMap<String, String> nativeObj = new HashMap<String, String>();
		if (CheckCftParameters() == false) {
			throw new SDKRuntimeException("生成package参数缺失！");
