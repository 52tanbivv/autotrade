...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		try {
			...
			if (user != null) {
				...
				Role role = accountService.findRole(user.getId());
				...
				ShiroUser suser = new ShiroUser(user.getId(), user.getLoginName(), user.getName(), user);
				String passwdSSO = String.copyValueOf(token.getPassword());

				return new SimpleAuthenticationInfo(suser, user.getPassword(), ByteSource.Util.bytes(salt), getName());
			} else {
				return null;
			}
		} catch (Exception e) {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		// if (user != null) {
		// info.addRoles(StringUtil.getNullStrToStringList(user.getRoles()));
		// }
		return info;
	}
	...
	public static class ShiroUser implements Serializable {
		...
		public ShiroUser(String id, String loginName, String name, User user) {
			this.id = id;
