...
public class DateUtil {
	...
	public static String formatDateTime(Object paramDate) {
		try {
			return DateUtil.formatDateTime((Timestamp) paramDate);
		} catch (Exception e) {
			e.printStackTrace();
			return "";
		}
	 ...
	 * @throws Exception
	 */
	public static boolean comparecurrentTime(String after_hhmmss, String before_hhmmss, String logTime)
			throws Exception {
		String _after_hhmmss_Str = new StringBuffer(DateUtil.fomartCurrentTime(DateUtil.COMPACT_DATE_FORMAT))
				.append(after_hhmmss).toString();
		Date _after_hhmmss_Date = DateUtil.stringToDate(_after_hhmmss_Str, "yyyyMMddHHmmss");
		Calendar after_hhmmssCalendar = Calendar.getInstance();
		after_hhmmssCalendar.setTime(_after_hhmmss_Date);

		String _before_hhmmss_Str = new StringBuffer(DateUtil.fomartCurrentTime(DateUtil.COMPACT_DATE_FORMAT))
				.append(before_hhmmss).toString();
		Date _before_hhmmss_Date = DateUtil.stringToDate(_before_hhmmss_Str, "yyyyMMddHHmmss");
		...
		// 进行早上9点操作判断
		if (currentCalendar.before(before_hhmmssCalendar) && currentCalendar.after(after_hhmmssCalendar)) {
			return true;
		} else {
			return false;
		}
	...
	public static List<Object[]> getBetweenRq(String startTime, String endTime) {
		...
		start.set(calendarStartTime.get(Calendar.YEAR), calendarStartTime.get(Calendar.MONTH),
				calendarStartTime.get(Calendar.DAY_OF_MONTH));
		Calendar end = Calendar.getInstance();
		end.set(calendarEndTime.get(Calendar.YEAR), calendarEndTime.get(Calendar.MONTH),
	...
	public static int getWeekInx(Date smdate, Date bdate) {
		...
		// bdate = getAfterCurrentDate(bdate, 1);
		// }
		double num = (double) (bdate.getTime() - smdate.getTime()) / (24 * 3600 * 1000 * 7);
	...
	public static String daysBetweenForSpecial(String datetime) {
		...
		String dayStr = "";
		try {
			interval = daysBetween(datetime.split(" ")[0], DateUtil.getCurrentYYYYTime());
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public static List<Object[]> getPreviousMonth(int lastMonth, String datePattern) {
		...
		for (int i = lastMonth - 1; i >= 0; i--) {
			...
			Object[] obj = { 12 - i, DateUtil.dateToString(cal.getTime(), datePattern) };
			returnList.add(obj);
		}
		return returnList;
	}
	...
	public static long getCurrentStamp() {
		...
		return ((year - 1970) * 12 * 31 + mon * 31 + (day - 1)) * (24 * 60 * 60) + (hour * 60 + min) * 60 + sec;
	}
	 ...
	 */
	public static boolean compareTimeIsInInterval(String time, String begin, String end) {
	...
	private static int[] convertStringArrToIntArr(String[] source) throws NumberFormatException {
	...
	public static Map<String, String> getCurrentStartDatetimeAndEndDatetime(String currentDate) {
		Map<String, String> map = new HashMap<String, String>();
	...
	public static java.sql.Date getCurrentSqlDate() {
		try {
			...
			e.printStackTrace();
			throw new BusinessException("日期转换错误", e);
		}
	}
	 ...
	 * @return
	 */
	public static boolean checkIsBetweenDates(String startDate, String endDate, String currentDate) {
	...
	public static Boolean comparisonTime(String time1, String time2) {
		...
		Long seconds1 = Long.valueOf(times1[0]) * 3600 + Long.valueOf(times1[1]) * 60;
		Long seconds2 = Long.valueOf(times2[0]) * 3600 + Long.valueOf(times2[1]) * 60;
	 ...
	 * @return
	 */
	public static final List<String> fetchDateList(String beginDatetimeStr, String endDatetimeStr) {
		...
		if (days > 0) {
			for (int i = days - 1; i >= 0; i--) {
				...
				dateStrList.add(DateUtil.convertDatetimeStrToDateStr(DateUtil.formatDate(cal.getTime())));
			}
		}
		return dateStrList;
	}
	...
	public static String addTimeAfterDate(String publishdate) {
		...
		try {
			...
			if (publishdate.split(" ").length == 1) {
				publishdate = publishdate + " " + getCurrentAllTime().split(" ")[1];
			}
		} catch (ParseException e) {
	...
	public static boolean checkNowIsIn(String starttime, String endtime) {
		...
		Date now = new Date();
		if (startDatetime.getTime() <= now.getTime() && now.getTime() <= endDatetime.getTime()) {
			return true;
		}
		return false;
	}
	...
	public static boolean checkNowIsIn(String starttime, int days) {
		...
		if (startDatetime.getTime() <= now.getTime()
				&& now.getTime() <= startDatetime.getTime() + days * 24 * 60 * 60 * 1000) {
			return true;
		}
		return false;
	}

	public static String getTimeDiff(Date oldTime, Date newTime) {
		...
		if (newTime.getTime() < oldTime.getTime()) {
			return "";
		} else if (timeDiff > (1000 * 3600 * 24 * 30l)) {
			return (timeDiff / (1000 * 3600 * 24 * 30l)) + "月前";
		} else if (timeDiff > 1000 * 3600 * 24l) {
			return (timeDiff / (1000 * 3600 * 24l)) + "天前";
		} else if (timeDiff > 1000 * 3600) {
			return (timeDiff / (1000 * 3600)) + "小时前";
