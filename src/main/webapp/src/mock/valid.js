...
export function valid (template, data) {
...
var Diff = {
    ...
    switch (schema.type) {
      case 'string':
        ...
        if (schema.template.match(Constant.RE_PLACEHOLDER)) {
          if (schema.template.indexOf('@integer') === 0 || schema.template.indexOf('@natural') === 0 || schema.template.indexOf('@float') === 0) {
            Assert.equal('type', schema.path, Util.type(data), 'number', result)
            return result.length === length
          } else if (schema.template.match('@boolean')) {
            Assert.equal('type', schema.path, Util.type(data), 'boolean', result)
            return result.length === length
          }
          return true
        }
  ...
  items: function (schema, data, name, result) {
    ...
    // 无生成规则
    if (!schema.rule.parameters) {
      Assert.greaterThanOrEqualTo('items length', schema.path, data.length, schema.items.length, result)
    } else {
      // 有生成规则
      // |min-max
      if (rule.min !== undefined && rule.max !== undefined) {
