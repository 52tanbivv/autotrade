...
K.options = {
	...
	items : [
	 		...
	 		'media', 'insertfile', 'table', 'hr', 'emoticons', 'baidumap', 'pagebreak','link','unlink','|', 'about'
	 	],//'flash',
//	items : [
...
function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
	...
	var videoReplaceStr = "VIDEO189889842989289348234"
	if (html.indexOf("<video ") >= 0)
	{
		var videoStartInx =  html.indexOf("<video ");
		var videoEndInx = html.indexOf("</video>");
		
		var videoLength = videoEndInx + 8 - videoStartInx;
		
		var videoHtml = html.substr(videoStartInx,videoLength);
		 
		html = html.replace(videoHtml, videoReplaceStr);
	}
	
	var audioReplaceStr = "AUDIO84271823412341732194123984"
		
	if (html.indexOf("<audio ") >= 0)
	{
			var audioStartInx =  html.indexOf("<audio ");
			var audioEndInx = html.indexOf("</audio>");
			
			var audioLength = audioEndInx + 8 - audioStartInx;
			
			var audioHtml = html.substr(audioStartInx,audioLength);
			 
			html = html.replace(audioHtml, audioReplaceStr);
	}
	...
	html = html.replace(videoReplaceStr, videoHtml);
	html = html.replace(audioReplaceStr, audioHtml); 
	return _trim(html);
}
...
function _mediaTypeVideoAndAudio(src) {
...
function _mediaEmbed(attrs) {
	var VideoAndAudio=_mediaTypeVideoAndAudio(attrs.src);
	
	if (VideoAndAudio == "video") {
		return _mediaEmbedVideo(attrs);
	} else if (VideoAndAudio == "audio") {
		return _mediaEmbedAudio(attrs);
	} else {
 ...
 * LINGCHEN 20160118
 */
function _mediaEmbedVideo(attrs) {
	var VideoAndAudio=_mediaTypeVideoAndAudio(attrs.src);
	var html = '<'+VideoAndAudio+' ';
	_each(attrs, function(key, val) {
		if (key == 'autostart' || key == 'autoplay') {
			if (val == 'true' || val == 'autoplay') {
				html += 'autoplay="autoplay" ';
			}
		}
	});
	html += 'style' + '="width:100%;height:' + attrs.height+'" ';
	
	html += ' controls="controls">';
	html += _mediaSourceHtml(attrs);
	html += '</'+VideoAndAudio+'>';
 ...
 * LINGCHEN 20160118
 */
function _mediaEmbedAudio(attrs) {
	var VideoAndAudio=_mediaTypeVideoAndAudio(attrs.src);
	
	var endtype="?avthumb/mp3";
	
	if (attrs.src.indexOf("avthumb")>0) {
		endtype = "";
	}
	...
	var src  = 'src' + '="' + attrs.src +  endtype+'" ';
	
	var html = '<div><'+VideoAndAudio+' '+src+ ' controls="controls" ' + autostart + 'width="200" height="50">' + '</'+VideoAndAudio+'></div>';
	return html;
}

/**
 * 判断图片路径是否属于阿里的cdn static.weixiaotong.com.cn
 * @param qiniuUrl
 * @returns
 */
function checkQiniuUrlKinderedit(qiniuUrl) {
	var oscdn_domain = 'static.weixiaotong.com.cn';
	if (qiniuUrl.indexOf(oscdn_domain) > 0 && qiniuUrl.indexOf(".") > 0 ) {
		if(qiniuUrl.indexOf("@")>0){
			return qiniuUrl.split("@")[0] + '@700w_700h_80q.src'+ "|" + qiniuUrl.split("@")[1] ;
		}else{
			return qiniuUrl + '@700w_700h_80q.src' ;
		}
	} else {
		return qiniuUrl;
	}
...
function _mediaSourceHtml(attrs) {
...
_extend(KCmd, {
	...
	insertimage : function(url, title, width, height, border, align) {
		...
		var imageurl=checkQiniuUrlKinderedit(_escape(url));
		var html = '<img src="' + imageurl + '" data-ke-src="' + imageurl + '" ';
		var initWidth=600;//设置最大宽度
		var initHeight=height;
		if (width) {
			if(width > initWidth){
		    	 var scaling = 1-(width-initWidth)/width;    
		         //计算缩小比例
		         //objImg.width = width*scaling;
		    	 width=initWidth;
		         initHeight = height*scaling;    //img元素设置高度时需进行等比例缩小
		    }
			html += 'width="' + width + '" ';
		}
		if (height) {
			
			//html += 'height="' + initHeight + '" ';
		}
		if (title) {
...
_extend(KUploadButton, {
	init : function(options) {
		...
		var html = [
			...
			'</span>',
			'<input type="file" class="ke-upload-file" id="' + fieldName + '" name="' + fieldName + '" tabindex="-1" />',
...
KindEditor.lang({
	...
	strikethrough : '删除线',
	removeformat : '删除格式',
	image : '图片(jpg,jpeg,png)每张限制5M',
	multiimage : '批量图片上传(jpg,jpeg,png)每张限制5M',
	flash : 'Flash',
	media : '视音频(mp4/mp3) 限制(100M)',
	table : '表格',
	tablecell : '单元格',
	hr : '插入横线',
	emoticons : '插入表情',
